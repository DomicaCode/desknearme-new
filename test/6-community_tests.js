...
test.page(myUrl + '/search')('Chat to merchant', async (t) => {
  await t.useRole(buyerRole)
  .typeText(itemSearch.search.keyword, SellerMerchantB.item)
  .click(Selector('a').withText(SellerMerchantB.item))
  .click(Selector('a').withText('Contact seller'))
  .wait(3000)
  .typeText(Selector('input[placeholder="Type your message here and press Enter to send"]'), "Hi Merchant, i want to ask about " + SellerMerchantB.item)
  .pressKey('enter')
  .wait(1000)
  .expect(Selector('p').withText('Hi Merchant, i want to ask about ' + SellerMerchantB.item).exists).ok()
})
